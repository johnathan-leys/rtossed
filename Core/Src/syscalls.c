...
_ssize_t _read_r(struct _reent *ptr, int fd, void *buf, size_t cnt){//need to write code to call HAL, get char from huart3, get 1 char and return
...
HAL_UART_Receive( &huart3, charStore, 1, HAL_MAX_DELAY);// pass ptr to huart3, store in charStore, 1 char, wait for max time allowed
